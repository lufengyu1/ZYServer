...
question.get('/question', async(req, res) => {
    ...
    res.send({ result: { total, pageNum, questionList: result }, meta: { status: 200, des: 'Success' } });
})

question.put('/insert', async(req, res) => {
    let result = await QuestionDB.insertMany({ id: req.body.id, operation: req.body.operation, name: req.body.name, quantity: req.body.quantity, reason: req.body.reason });
    if (result) {
