...
register.get('/register', async(req, res) => {
    const { pageNum, pageSize, pageNum1, pageSize1, type, num } = req.query;
    let sort = {};
    sort[type] = num - 0;
    if (!pageNum || !pageSize || !pageNum1 || !pageSize1) return res.send({ result: null, meta: { status: '404', des: '参数错误' } });
    let result = await RegisterDB.find({}).sort(sort);
    ...
    result = await RegisterDB.find({}).sort(sort).limit(pageSize - 0);
